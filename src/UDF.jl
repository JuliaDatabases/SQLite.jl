...
sqlreturn(context, val::AbstractString) = C.sqlite3_result_text(context, val, sizeof(val), C.SQLITE_TRANSIENT)
sqlreturn(context, val::Vector{UInt8})  = C.sqlite3_result_blob(context, val, sizeof(val), C.SQLITE_TRANSIENT)
