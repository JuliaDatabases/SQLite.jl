...
# UnknownSchemaTable is a minimal Tables.jl source used by the `load!`
# schema-handling tests; defined at top level so its method extensions are
# available to every testset below.
...
@testset "SQLite" begin
    ...
    @testset "DB open/close" begin
        @testset "DB Connection" begin
            dbfile = joinpath(
                dirname(pathof(SQLite)),
        ...
        @testset "Issue #158: Missing DB File" begin
            @test_throws SQLiteException SQLite.DB(
                "nonexistentdir/not_there.db",
            )
        end
        ...
        @testset "closed database errors" begin
            ...
            tbl_a =
                DBInterface.execute(db_a, "select myfunc(1) as x") |>
                columntable
            tbl_b =
                DBInterface.execute(db_b, "select myfunc(1) as x") |>
                columntable
            @test tbl_a.x == [2]
            @test tbl_b.x == [10]
    ...
    @testset "tables interface" begin
        @testset "SQLite.tables(db)" begin
            setup_clean_test_db() do db
                ...
                @test SQLite.DBTable("Album") ==
                      SQLite.DBTable("Album", nothing)
    ...
    @testset "execute / query" begin
        ...
        @testset "DBInterface.execute([f])" begin
            setup_clean_test_db() do db
                ...
                employees_stmt =
                    DBInterface.prepare(db, "SELECT * FROM Employee")
                @test isequal(
        ...
        @testset "Prepared Statements" begin
            setup_clean_test_db() do db
                DBInterface.execute(
                    db,
                    "CREATE TABLE temp AS SELECT * FROM Album",
                )
                r =
                    DBInterface.execute(
                        db,
                        "SELECT * FROM temp LIMIT ?",
                        [3],
                    ) |> columntable
        ...
        @testset "query iteration protocol" begin
            # https://github.com/JuliaDatabases/SQLite.jl/issues/251
            # A Query row is only valid while it is the current cursor position;
            # advancing the cursor invalidates the previously-returned row.
            # The table is recreated here (the original suite reused the table
            # built by the issue #259 test above) so this testset is self-contained.
            db = SQLite.DB()
    ...
    @testset "transactions" begin
        ...
        @testset "Issue #341: load! inside transaction" begin
            ...
            result =
                DBInterface.execute(db, "SELECT * FROM test_outside") |>
                columntable
            @test result.a == [1, 2, 3]
            @test result.b == ["x", "y", "z"]
            ...
            result2 =
                DBInterface.execute(db, "SELECT * FROM test_inside") |>
                columntable
            ...
            DBInterface.transaction(db) do
                ...
                @test DBInterface.execute(db, "SELECT * FROM test_rollback") |>
                      columntable |>
                      x -> x.e == [100]
            end
    ...
    @testset "load! / drop!" begin
        @testset "Remove Duplicates" begin
            setup_clean_test_db() do db
                ...
                dt3 =
                    DBInterface.execute(db, "Select * from temp") |> columntable
                @test dt3[1][1] == 1
                @test dt3[2][1] == "A"
                @test dt3[1][2] == 2
                @test dt3[2][2] == "B"
        ...
        @testset "load!() column names escaping" begin
            db = SQLite.DB()
            tbl =
                NamedTuple{(:a, Symbol("50.0%"))}(([1, 2, 3], ["a", "b", "c"]))
        ...
        @testset "Bool column data" begin
            ...
            r =
                DBInterface.execute(db, "SELECT * FROM bool_data") |>
                columntable
            @test isequal(r, (a = [1, 0, 0], b = [0, missing, 1]))
            SQLite.drop!(db, "bool_data")
        end

        @testset "load! with unknown schema source" begin
        ...
        @testset "PR #302: load! on_conflict clauses" begin
    ...
    @testset "strict mode" begin
        ...
        @testset "Symbol in TEXT column under strict mode" begin
            db = SQLite.DB()
        ...
        @testset "Symbol in BLOB column under strict mode" begin
        ...
        @testset "Symbol in TEXT column, non-NULL first row" begin
    ...
    @testset "UDF registration" begin
        @testset "Register functions via prepared statements" begin
            setup_clean_test_db() do db
                ...
                @test_throws AssertionError SQLite.register(
                    db,
                    triple,
                    nargs = 186,
                )
        ...
        @testset "Register scalar and aggregate functions" begin
            setup_clean_test_db() do db
                SQLite.@register db add4
                r =
                    DBInterface.execute(
                        db,
                        "SELECT add4(AlbumId) FROM Album",
                    ) |> columntable
                s =
                ...
                s =
                    ...
                    columntable
                @test abs(r[1][1] - 2 * sum(convert(Vector{Float64}, s[1]))) <
                      0.02
                ...
                @test filter(x -> x.name == "tab2", SQLite.tables(db2)) |>
                      length == 0
    ...
    @testset "serialization" begin
        @testset "serialization edgecases" begin
            db = SQLite.DB()
            r =
                DBInterface.execute(db, "SELECT zeroblob(2) as b") |>
                columntable
            @test first(r.b) == [0, 0]
            r =
                DBInterface.execute(db, "SELECT zeroblob(0) as b") |>
                columntable
            @test first(r.b) == []
        end
        ...
        @testset "ReinterpretArray" begin
            binddb = SQLite.DB()
            DBInterface.execute(binddb, "CREATE TABLE temp (b BLOB)")
            DBInterface.execute(
                binddb,
                "INSERT INTO temp VALUES (?)",
